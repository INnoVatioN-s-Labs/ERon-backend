...
public class CharacterMetaStatRepository {
    ...
    private int insertSample(String sourceUserId, UserGameSummary game) {
        ...
        return jdbcTemplate.update(connection -> {
            ...
            setInteger(statement, 8, game.gameRank());
            setInteger(statement, 9, game.playerKill());
            setInteger(statement, 10, game.playerAssistant());
            setInteger(statement, 11, game.playerDeaths());
            setInteger(statement, 12, game.damageToPlayer());
            setInteger(statement, 13, game.teamKill());
            setInteger(statement, 14, game.rankPoint());
            setInteger(statement, 15, game.mmrGain());
            statement.setString(16, game.startDtm());
            statement.setTimestamp(17, Timestamp.from(Instant.now(clock)));
            return statement;
        }, keyHolder);
    }
